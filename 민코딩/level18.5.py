...
#
# for i in range(len(A_asci)):
#     result_list[cnt_list[A_asci[i]]-1] = A_asci[i]
#     cnt_list[i] -= 1
#
...
# for i in range(len(cnt_list)):
#     if cnt_list[i] > 0:
#         print('{}:{}'.format(chr(i), cnt_list[i]))

# 민코딩 level 18.5 11번
# ghost = list('GHOST')
# ghost_asci = []
# for i in range(len(ghost)):
#     ghost_asci.append(ord(ghost[i]))
#
# A = list(input())
# cnt_list = [0]*100
#
...
#
#
# for i in range(len(ghost_asci)):
#     cnt_list[ghost_asci[i]] -= 1
#
# # 입력받은 문자에서 GHOST만큼 cnt 횟수차감
# flag = 0
# for i in range(len(cnt_list)):
#     if cnt_list[i] < 0:
#         flag = 0
