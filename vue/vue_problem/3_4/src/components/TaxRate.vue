<template>
  <div>
    <h2>산출세액계산</h2>
    <p> 산출세액: {{ afterTax }} 만원</p>
    <p> 세액 감면: (-) {{ taxCredit }} 만원 </p>
    <hr>
    <p>결정세액: {{ finalTax }} 만원 </p>
  </div>
</template>
    ...
    computed: {
        afterTax() {
            if (this.taxBase <= 1200) {
                ...
                return Math.round(this.taxBase * 0.24 - 552)
        ...
        finalTax() {
            return this.afterTax - this.taxCredit >= 0 ? this.afterTax - this.taxCredit : 0
        }
